...
def eval_replica_dump(masks, gt_mask, id, save_dir):

    iou = cal_IoU(masks, gt_mask)
    acc = cal_ACC(masks, gt_mask)
    print('replica iou and acc for id {}: '.format(id), iou, acc)
    with open(os.path.join(save_dir, "IoU_instances.txt"), 'a') as f:
        f.write('id: {:03d}, iou: {:08f}, acc: {:08f}\n'.format(id, iou, acc))
    return


def cal_ACC(a, b):
    def num_elements(tmp):
        ret = 1
        for s in tmp.shape:
            ret *= s
        return ret
    acc = np.count_nonzero(a == b) / num_elements(a)
    return acc
...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0,
    ...
    for i, c2w in tqdm(enumerate(render_poses)):
        ...
        if savedir is not None:
            ...
            norm_dp = (depth - depth.min())/(depth.max() - depth.min())
...
def cal_IoU(a, b):
    ...
    Args:
        a: A array of shape (N, H, W).
        b: A array of shape (N, H, W).

    Returns:
        A np.array of shape (N,) containing the IoU score between each pair of
        elements in a and b.
    """
    ...
    union = np.count_nonzero(a + b) + 1e-7
...
def config_parser():
    ...
    parser.add_argument("--id_instance", type=int, default=1)
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        # check data range and type. image[N, H, W, C], poses [N, 3, 5], 
        hwf = poses[0, :3, -1]
        poses = poses[:, :3, :4]
        ...
        all_images, all_poses, render_poses, bds, hwf, i_split, all_depth_gts, idx_selected_all, masks, masks_gt = load_replica_data(args.datadir, id_instance=args.id_instance)
        print('Loaded replica', all_images.shape, render_poses.shape, hwf, args.datadir)
        i_train, i_val, i_test = i_split
        near, far = bds
        print('NEAR FAR', near, far)

        images = all_images[i_train] # N H W 3
        depth_gts = all_depth_gts[i_train] # N H W 
        poses = all_poses[i_train] # N 4 4
        ...
        print(f'Dino (2D) Accuracy is {acc} and IoU is {iou}')
    ...
    if args.render_test:
        render_poses = np.array(all_poses[i_test])
    elif args.render_train:
        render_poses = np.array(all_poses[i_train])
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            if args.render_test:
                ...
                images = all_images[i_test]
            else:
                # Default is smoother render_poses path
                images = all_images[i_train]
            ...
            if args.render_test_ray:
                ...
                rgbs, disps, _, _ = render_path(render_poses, hwf, args.chunk, render_kwargs_test, gt_imgs=images,
    ...
    if use_batching:
        ...
        if args.colmap_depth:
            ...
            for i in range(len(i_train)):
                ...
                # use gt depth 
                rays_depth = np.stack(get_rays_np(H, W, focal, poses[i, :3, :4])).reshape(2, -1, 3)
                rays_depth = np.transpose(rays_depth, [1, 0, 2])
                depth_value = np.repeat(depth_gts[i].flatten()[:, None, None], 3, axis=2)  # N x 1 x 3
                weights = np.ones_like(depth_value)
                rays_depth = np.concatenate([rays_depth, depth_value, weights], axis=1)  # N x 4 x 3
                rays_depth_list.append(rays_depth)
    ...
    for i in trange(start, N_iters):
        ...
        #         depth_loss = img2mse(depth_col[depth_mask], target_depth[depth_mask])
        ...
        if i % 1000 == 0 and i > 0:
            with torch.no_grad():
                idx = np.random.choice(range(len(i_train)))
                rgbs, disps, probs, _ = render_path(poses[idx:idx+1], hwf,
                                                 args.chunk,
                                                 render_kwargs_test,
            ...
            plt.subplot(122)
            plt.imshow(1 / (disps[0] + 1e-6))
            plt.colorbar()

            plt.gcf().set_dpi(300)
            os.makedirs('test_renders', exist_ok=True)
            plt.savefig('test_renders/{}_rendered_{}.png'.format(
                expname, str(i)
            ), format='png')
            plt.clf()
        ...
        #     with torch.no_grad():
        #         rgbs, disps, probs, masks = render_path(poses, hwf, args.chunk, render_kwargs_test,  # todo change to poses
        #                                      only_object=args.object_removal,
        #                                      render_factor=args.render_factor,
        #                                      render_mask=args.render_mask and not args.object_removal,
        #                                      post_opening=args.post_opening, savedir=os.path.join(basedir, expname),
        #                                      gt_imgs = images)
        ...
        #         ))
        #     imageio.mimwrite(moviebase + 'rgb.mp4', to8b(rgbs), fps=12, quality=8)
        #     imageio.mimwrite(moviebase + 'disp.mp4', to8b(disps / np.nanmax(disps)), fps=12, quality=8)
        #     imageio.mimwrite(moviebase + 'mask.mp4', to8b(masks), fps=12, quality=8)
        ...
        #     print(f'the final Accuracy is {acc} and IoU is {iou}')
            # accuracies = []
            # IoUs = []
    ...
    with torch.no_grad():
        rgbs, disps, probs, masks = render_path(poses, hwf, args.chunk, render_kwargs_test,  # todo change to poses
                                        ...
                                        post_opening=args.post_opening, savedir=os.path.join(basedir, expname),
                                        gt_imgs = images)   
    eval_replica_dump(np.stack(masks), masks_gt, args.id_instance, os.path.join(basedir, expname))
