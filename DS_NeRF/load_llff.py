...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, object_removal=False):
    ...
    # imgdir = os.path.join(basedir, 'images' + sfx, 'original_images')
    imgdir = os.path.join(basedir, 'images' + sfx)
    if object_removal:
        mskdir = os.path.join(basedir, 'images' + sfx + '/label_mv_bootstrapped')
    else:
        mskdir = os.path.join(basedir, 'images' + sfx + '/label_mv_bootstrapped')  # todo change to /dino
    mskgtdir = os.path.join(basedir, 'images' + sfx + '/label')  # todo change to label
    if not os.path.exists(imgdir):
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False,
                   spherify_hack=True, object_removal=False):
    poses, bds, imgs, masks, masks_gt, mask_indices = _load_data(basedir, factor=factor, object_removal=object_removal)  # factor=8 downsamples original imgs by 8x
